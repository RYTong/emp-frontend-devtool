...
class LogView extends React.Component {
  ...
  setPeerName(key) {
    if (Reflect.has(this.state.peers, key)) {
     let device, [ip, port] = key.split('#')
     if (device = getDeviceByAddress('logger', parseIP(ip), parseInt(port))) {
       this.setState({peers: { ...this.state.peers, [key]:device}})
       log('set peer#', key, 'to', device)
     } else {
       setTimeout(() => this.setPeerName(key), 200)
     }
    }
  }

  peer2name(key) {
    let device = this.state.peers[key]
  ...
  listen() {
   ...
   emitter.on('peer-connect', key => {
     ...
     this.setState({peer: this.state.peer || key,
                    peers: { ...this.state.peers, [key]: null }
                  })
   })
   emitter.on('peer-disconnect', (key) => {
     let peers = this.state.peers
     let peer = this.state.peer === key ? null : this.state.peer
...
const DeviceDisplay = (props) => {
  console.log('render DeviceDisplay:', props)
  if (_.size(props.peers) > 1) {
    return (
