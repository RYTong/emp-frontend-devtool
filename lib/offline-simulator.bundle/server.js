...
import { WWW_PATH, PRELOAD_PATH, H5_PATH, COMMON_PATH } from '../constants'
...
let project, wwwpath, preloadpath, h5path, commonpath
...
export default {
  ...
  start(proj) {
    ...
    commonpath  = path.join(project, COMMON_PATH)
...
function process(req, res) {
  ...
  if (actionType === 'reload' || actionType === 'restart') {
    loggerEmitter.emit('clear', token)
  }
