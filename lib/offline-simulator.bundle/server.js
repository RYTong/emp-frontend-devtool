...
let error = require('../log-prefix')('[offline]', 'error')
...
export default {
  ...
  changeSyncJump(){
    // console.log("do change--------:", bSyncJump);
    bSyncJump = !bSyncJump;
  }
...
const registerAccessPoint = app => {
  ...
  app.all('/state', (req, res) => {
    ...
    res.json({
      ...
      'offline-version': ap.detectOfflineVersion(project),
...
const processOffline = (req, res) => {
  ...
  if (reqpath.startsWith('channels/')||reqpath.startsWith('channels\\')) {
    realpath = ap.offlineToAbsolute(reqpath, project)
  } else if (reqpath.startsWith('preload/')||reqpath.startsWith('preload\\')) {
    ...
    if (!(reqpath.includes('/')||reqpath.includes('\\'))) {
      // console.log("-------------------1");
      realpath = ap.offlineToAbsolute(reqpath, project)
    } else if (reqpath.startsWith('--efd-files--/')||reqpath.startsWith('--efd-files--\\')) {
      // console.log("------------------2-");
  ...
  if (fileext === '.xhtml' || fileext === '.div' || fileext === '.html') {
    ...
    if (bSyncJump && isNewPage && fileExists) {
