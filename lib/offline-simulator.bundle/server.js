...
export default {
  ...
  start(project) {
    ...
    // check if specified project is valid
    // if (!isValidProject()) {
    ...
    detectPort(_port)
      .then(port => {
        if (_port !== port) {
          ...
          _server.on('close', () => {
            _server = null;
            dispatch(closePort('offline-resource'));
            serverEmitter.emit('status', 'simulator', 'ready');
          });
          _server.on('error', (e) => {
          ...
          dispatch(openPort('offline-resource', _port))
...
function registerAccessPoint(app) {
  ...
  app.all('/heartbeat', (req, res) => {
    ...
    // Good boy, now I will give you a lollipop ^_^
    let lollipop = {
      version: pkginfo.version,
      ip: localIP.v4(),
      serverPorts: getState().servicePorts
    };

    // for (let server in serverEmitter.status) {
    //   if (serverEmitter.status[server] === 'running') {
    //     lollipop.serverPorts[server] = serverEmitter.ports[server];
    //   }
    // }
