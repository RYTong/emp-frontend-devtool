...
import { PRELOAD_PATH, H5_PATH, HOME_PAGE } from '../constants'
...
let _server = null
const _port = 6102
const _mainbuff = fs.readFileSync(path.join(__dirname, 'main.xhtml'))
...
function process (req, res) {
  ...
  if (reqpath.startsWith('channels/')) {
    ...
    if (reqpath.startsWith('--efd-files--')) {
      realpath = ap.getFilePath(path.basename(reqpath))
    } else {
  ...
  offlinePath = ap.absoluteToOffline(realpath, project)
