...
import { absoluteToOffline, resolveCommonPath } from '../app-path'
...
export default {
  ...
  notify (absolutePath) {
    ...
    let devices = _.map(
      _socks,
      (socket, key) => getDeviceName(key)
    ).join(' ')

    log('notify', data, 'to', _.size(_socks), 'clients:', devices)
...
let getDeviceName = key => {
  let [ip, port] = key.split('#')
  let device = getDeviceByAddress('hot-loader', ip, port)
...
let isGlobalFile = (filename) => {
  ...
  if (path.extname(filename) === '.lua') {
    if (_project) {
      let globalLuaPath = path.join(resolveCommonPath(_project), 'lua')
