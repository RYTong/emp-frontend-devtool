...
const addCommand = (selector, command, filepath, key) => {
  if (commands[command]) {
    commands[command].dispose()
  }

  commands[command] = atom.commands.add(selector, {
  ...
  created (event) {
    ...
    if (isRunning && isAppFile && hasPeer) {
      ...
      if (key) {
        ...
        this.label = `在${getPeerDesc(key, peers[key])}中打开${bn}`
        this.command = command
        this.visible = true

        addCommand(selector, command, filepath, key)
      }
    }
  }
})
