...
import { toggleApp, changeSyncJump } from './actions'
...
let name, action, refresh, _refresh, device,
  qr, expand, compress, state, project, syncAction;
const sizes = ['12px', '16px', '24px', '32px']

const draw = (toolBar) => {
  hijack(toolBar.toolBarView)
  ...
  // auto jump
  syncAction = toolBar.addButton({
    icon: 'sync-off',
    iconset: 'mdi',
    tooltip: '开启文件进入时的自动跳转',
    callback: () =>{
      toggleActionSync();
      store.dispatch(changeSyncJump())

    }
  })
  // $(syncAction.element).addClass('syncAction')
...
const toggleActionSync = () => {
  ['mdi-sync-off', 'mdi-sync'].forEach((sClass)=>{
    $(syncAction.element).toggleClass(sClass);
  });
