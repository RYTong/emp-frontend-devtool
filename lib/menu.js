...
let name, action, log, dbg, refresh, exec, device,
...
const draw = (toolBar) => {
  ...
  refresh = toolBar.addButton({
    icon: 'refresh',
    iconset: 'mdi',
    callback: 'emp-frontend-devtool:open-file-in-device',
    tooltip: '刷新当前页面到设备'
  })
  $(refresh.element).addClass('disabled')
...
const toggleAction = () => {
  ...
  $(refresh.element).toggleClass('disabled')
